...
from file_share_app.api.serializers import FileListSerial
...
def pre_process_data2(request_data):
    ''' 
    converting byte(_io)-file-object in to django-file-object     
    '''
...
def upload_file(request):
    if request.method == 'POST':
        ...
        if serializer.is_valid():
            ...
            if '' in [v for i,v in data.items()]:
                return Response({'status : data not entered'})
            return Response({'status : issue in internal server'})
        if settings.ALLOWED_HOSTS:
            file_url = str(settings.ALLOWED_HOSTS[0])
        else:
            file_url = 'http://127.0.0.1:8000'
        message = {
            "success":f"sharable link of uploaded file is {file_url+'/share/download/'+serializer.data['user_token']}   "
        }
